      ...
      if (offCtx) {
        if (images) {
          ...
          img.onload = () => {
            ...
            offCtx.beginPath();
            offCtx.arc(32, 32, 28, 0, Math.PI * 2);
            ...
            offCtx.drawImage(img, 14, 14, 36, 36);
  ...
  useEffect(() => {
    ...
    if (canvas && ctx) {
      const animate = () => {
        ...
        const speed = 0.004 + (distance / maxDistance) * 0.01;
  ...
  return (
    <div className={cn("relative flex items-center justify-center", className)}>
      <canvas
        ...
        className="max-w-full cursor-grab active:cursor-grabbing rounded-full"
      ...
      {showControl && (
        <button
          ...
          className="absolute top-2 right-2 rounded-full border border-border bg-background/80 p-2 text-foreground backdrop-blur-sm transition-colors hover:bg-background"
        >
